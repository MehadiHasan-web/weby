            ...
            <form action="{{ route('student.update', $student->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row row-cols-2 mt-2">
                    <div class="mb-3">
                        ...
                        <input name="name" value="{{ old('name', $student->name ?? '') }}" type="name"
                            class="form-control" id="name" placeholder="Write student name..">
                        @error('name')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input name="email" value="{{ old('email', $student->email ?? '') }}" type="text"
                            class="form-control" id="email" placeholder="Write student email..">
                        @error('email')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="row row-cols-2 mt-2">
                    <div class="mb-3">
                        ...
                        <input name="institute_name" value="{{ old('institute_name', $student->institute_name ?? '') }}" type="text" class="form-control" id="institute_name"
                    ...
                    <div class="mb-3">
                        ...
                        <input name="student_class" value="{{ old('student_class', $student->student_class ?? '') }}" type="text" class="form-control" id="student_class"
                ...
                <div class="row row-cols-2 mt-2">
                    <div class="mb-3">
                        ...
                        <input name="roll" value="{{ old('roll', $student->roll ?? '') }}" type="number" class="form-control" id="roll"
                            placeholder="Write institute name.." required>
                        @error('roll')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
